...
const output = fs.createWriteStream(path.join(__dirname, 'pattern-auto-tab-grouper-v1.2.0.zip'));
const archive = archiver('zip', {
  zlib: { level: 9 } // Maximum compression
});

output.on('close', function() {
  console.log(`✅ Created pattern-auto-tab-grouper-v1.2.0.zip (${archive.pointer()} bytes)`);
